...
import { CyanButton, OutlineButton } from '@/components/ui'
    ...
    <section className="relative min-h-screen flex items-center overflow-hidden">
      ...
      <div className="absolute inset-0 z-0">
        ...
        <img 
          ...
          loading="eager"
        />
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_30%_50%,rgba(34,211,238,0.15),transparent_60%)]" />
      </div>

      {/* Decorative lines */}
      <div className="absolute left-0 top-0 bottom-0 w-px bg-gradient-to-b from-transparent via-cyan/20 to-transparent z-20" />
      <div className="absolute right-8 top-1/4 bottom-1/4 w-px bg-gradient-to-b from-transparent via-cyan/10 to-transparent z-20 hidden lg:block" />

      {/* Content */}
      <div className="relative z-20 max-w-7xl mx-auto px-6 pt-24 pb-16">
        <div className="max-w-3xl">
          {/* Eyebrow */}
          <p className="font-mono text-cyan text-2xl md:text-4xl lg:text-5xl font-bold tracking-tight uppercase mb-8 opacity-0 animate-slide-up"
          ...
          <div className="flex flex-wrap gap-4 opacity-0 animate-fade-in"
            ...
            <CyanButton 
              href={YOUTUBE_URL} 
              className="min-w-[220px] px-12 py-4 flex justify-center items-center bg-transparent border border-cyan-400 text-cyan-400 hover:bg-red-600 hover:border-red-600 hover:text-white transition-all duration-300"
            >
               {t('music.youtube')}
            </CyanButton>
          ...
          <div className="flex items-center gap-6 mt-12 opacity-0 animate-fade-in"
            ...
            {['Instagram', 'Facebook', 'TikTok', 'YouTube'].map((s) => (
              <a
                key={s}
                href="#"
                className="font-mono text-xs text-white/30 hover:text-cyan transition-colors tracking-widest uppercase"
              >
                {s}
              </a>
            ))}
      ...
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-20 flex flex-col items-center gap-2 animate-bounce">
        <div className="w-px h-10 bg-gradient-to-b from-transparent to-cyan/60" />
